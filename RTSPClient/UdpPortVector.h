		...
		unsigned short GetFreeUDPPort()
		{
			...
			{	
				if(udpportvector[i].bfree){
					...
					if( ( index < 0 ) || (udpportvector[index].iUserCount > udpportvector[i].iUserCount) )
					{
						index = i;
					}
				}
			...
			if( index >= 0 )
			{
				udpportvector[index].bfree = false;
				udpportvector[index].iUserCount++;
				return udpportvector[index].udpport;
			}
