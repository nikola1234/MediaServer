	...
	m_responseCodeInPlay(0)
{
	...
	memset( m_ServerAddressStr, 0 ,256 );
...
int CRTSPClient::sendPlayCommand(const char * url, float scale, const char* Range_clock )
{
	...
	sprintf(sendBuf, OptionsInfo, url, m_icseq, m_SessionID, scale, Range_clock, m_user_agent);
	m_iCommandStatus = PLAYCOMMAND;

	if(!m_recvThread_.joinable())
		m_recvThread_ = boost::thread(&CRTSPClient::handleRecvVideoDataThread , this);
...
Boolean CRTSPClient::handleSETUPResponse(char const* transportParamsStr)
{
	...
	while (sscanf(fields, "%[^;]", field) == 1) {
		if (sscanf(field, "server_port=%hu", &m_serverRTPUDPPort) == 1) {

		} else if (sscanf(field, "client_port=%hu", &clientRTPPort) == 1) {

		} else if (_strncasecmp(field, "source=", 7) == 0) {
			if(strlen(field + 7) < 256)strcpy(m_ServerAddressStr, field + 7);
	...
	if (isUMulticast && isRTP_UDP && m_serverRTPUDPPort && clientRTPPort && strlen(m_ServerAddressStr) > 0)
		return True;
	else
		return False;
	return False;
...
Boolean CRTSPClient::handleSDPResponse(char * sdpBodyStr)
{
	...
	while (1) {
		...
		} else if (checkForHeader(lineStart, "Content-Length:", 15, chParamsStr)) {
		} else if (checkForHeader(lineStart, "Content-Base:", 13, chParamsStr)) {
		}
	}
	...
	if (n <= 0 || n >= (int)camerainfo.length() - 1)return False;
...
int CRTSPClient::RecvPlayResponse()
{
	try{
		int i = 0;
		for (i = 0; i < 2500; i++)
