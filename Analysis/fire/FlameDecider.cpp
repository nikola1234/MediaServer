...
bool FlameDecider::judge(map<int, Target>& targets)
{
    ...
    for (map<int, Target>::iterator it = targets.begin(); it != targets.end(); it++) {
        ...
        if (isFlame /*&& it->second.times > 50*/) {
            flameDetected = true;
            rectangle(temp, it->second.region.rect, Scalar(0, 255, 0));
	   // printf("alarmRect.size()  is %d\n",alarmRect.size()); 
	   // printf("x= %d , y=%d ,width = %d , height = %d\n",it->second.region.rect.x,it->second.region.rect.y,it->second.region.rect.width,it->second.region.rect.height);
            alarmRect.push_back(it->second.region.rect);
        }
    }
   #ifdef DEBUG_OUTPUT
	    writer << temp; 
